...
class App:
    ...
    def _build_menu(self):
        ...
        m_help.add_command(label="📖  功能使用帮助", command=self.on_show_help)
        m_help.add_separator()
        m_help.add_command(label="ℹ  关于软件", command=self.on_show_about)
    ...
    def on_show_about(self):
        ...
        _mb.showinfo("关于绮绮采集器",
                     f"绮绮采集器  {lm.CLIENT_VERSION}\n\n"
                     "专为小红书内容运营设计的智能采集发布工具\n\n"
                     "功能涵盖：搜索采集 / 批量互动 / 爆品挖掘\n"
                     "          自动养号 / 笔记发布 / 自动搬运\n\n"
                     "如需帮助请查看菜单 ❓ → 功能使用帮助")
    ...
    def _show_help_dialog(self):
        ...
        try:
            import ttkbootstrap as ttkb
            top = ttkb.Frame(dlg, bootstyle="primary", padding=(16, 8))
        except Exception:
            top = ttk.Frame(dlg)
        top.pack(fill="x")
        try:
        ...
        ttk.Label(left, text="目  录", font=(FONT_UI, 11, "bold")).pack(anchor="w", pady=(0, 4))
        ttk.Separator(left).pack(fill="x", pady=(0, 4))

        lb = tk.Listbox(left, font=(FONT_UI, 11), activestyle="none",
                        selectbackground="#F25928", selectforeground="white",
        ...
        txt = tk.Text(right, font=(FONT_UI, 11), wrap="word",
        ...
        # ── 颜色标签 ──
        txt.tag_configure("h1",  font=(FONT_UI, 14, "bold"), foreground="#F25928", spacing1=6, spacing3=4)
        txt.tag_configure("h2",  font=(FONT_UI, 12, "bold"), foreground="#1a6faf", spacing1=10, spacing3=2)
        txt.tag_configure("h3",  font=(FONT_UI, 11, "bold"), foreground="#333")
        txt.tag_configure("tip", foreground="#0a7", font=(FONT_UI, 10))
        txt.tag_configure("warn",foreground="#c33", font=(FONT_UI, 10))
        txt.tag_configure("body",foreground="#444")
        txt.tag_configure("sep", foreground="#ccc")
        txt.tag_configure("step",foreground="#1a6faf", font=(FONT_UI, 10, "bold"))
...
【第二步：添加账号并登录】
  左侧面板点击"+ 新账号" → 输入账号名称 → 确认。
  选中账号 → 点"▶ 登录"→ 浏览器自动打开小红书扫码页面 → 微信/手机扫码 → 登录成功后窗口自动关闭。
  首次登录建议先用"⑦ 自动养号"跑 30 分钟热身。

【第三步：选功能，开始采集/操作】
  选中账号 → 切换到对应功能标签页 → 填写参数 → 点执行按钮。
  结果实时出现在右侧结果面板，可勾选后批量操作。

⚠ 注意：所有操作以"当前选中账号"为准，开始前请确认左侧列表中已高亮选中了正确的账号。"""),

            ("👤 账号管理",
             """【账号管理】

◆ 功能说明
  管理多个小红书账号的登录状态、代理和浏览器会话。

◆ 操作按钮说明
  + 新账号   → 新建一个账号配置（只建档，不登录）
  ▶ 登录     → 打开浏览器让你扫码，登录成功后自动保存 Cookie
  ▶▶ 全部   → 依次登录账号列表中所有账号
  ⏹ 停该号  → 停止当前选中账号的正在运行的任务
  ⏹ 全停    → 停止所有账号的所有任务
  🌐 代理    → 为当前账号单独配置 HTTP/SOCKS5 代理
  🔍 检测IP  → 在当前账号的浏览器中访问测速页，确认代理 IP 生效
  ✕ 关窗口  → 关闭当前账号的浏览器窗口（保留 Cookie）
...
⚠ 注意
  · 每个账号的 Cookie 存在本地文件，删除账号会一并删除。
  · 同一台机器建议同时运行账号 ≤ 5 个，避免 IP 被关联风控。
  · 代理建议每个账号独享一个 IP，不要多账号共用同一代理。"""),

            ("① 搜索采集",
             """【① 搜索采集】

◆ 功能说明
  按关键词搜索小红书笔记，批量采集标题、点赞/评论/收藏数、作者信息等，可导出 Excel。

◆ 使用步骤
  1. 切换到"① 搜索"标签页
  2. 输入关键词（支持多个词，建议用具体词如"显瘦穿搭 秋冬"）
  3. 填写采集数量（建议单次 50~200 条）
  4. 点击"🔍 搜索"按钮
  5. 结果显示在右侧面板，勾选后可批量点赞/关注/评论

◆ 导出
  结果面板上方点"导出 Excel"，文件保存到输出目录（菜单 📦备份 → 打开输出目录）

⚠ 注意
  · 搜索结果受账号登录状态影响，未登录可能只返回有限结果。
  · 开启"搜索时使用 IP 代理"选项可降低频率限制风险。
  · 单日同账号搜索建议不超过 500 次，间隔 2~5 秒/次。"""),

            ("② 笔记+评论采集",
             """【② 笔记+评论采集】

◆ 功能说明
  输入笔记 URL 或 ID，采集笔记详情（正文/图片/数据）和全部评论。

◆ 使用步骤
  1. 在小红书打开目标笔记，复制浏览器地址栏 URL（或笔记末尾 ID）
     URL 格式：https://www.xiaohongshu.com/explore/xxxxxxxx
  2. 粘贴到"笔记 URL\\ID"输入框
  3. 填写"采集评论数"（0 = 不采集评论，留空默认全采）
  4. 填写"评论最小/最大楼层"可按楼层区间过滤
  5. 点击"📄 采集笔记" 或 "💬 采集评论"

◆ 批量采集
  支持一次粘贴多个 URL，每行一条，自动队列处理。

⚠ 注意
  · 评论超过 1000 条的笔记采集时间较长，请耐心等待。
  · 部分限流笔记需要账号有一定的粉丝量或活跃度才能看到全部评论。
  · 评论数据仅供分析，请勿用于骚扰或批量私信。"""),

            ("③ 用户主页采集",
             """【③ 用户主页采集】

◆ 功能说明
  输入用户主页 URL 或用户 ID，采集该用户发布的所有笔记列表及基本信息。

◆ 使用步骤
  1. 打开目标用户主页，复制 URL 或用户 ID（URL 末尾数字字母串）
  2. 粘贴到"用户 URL\\ID"输入框
  3. 填写"采集数量"（留空表示全采）
  4. 点击"👤 采集主页"

◆ 意向用户分析
  采集完成后可点"🎯 意向用户分析"，系统会根据评论关键词（如"哪里买""怎么买"等）
  自动筛选出有购买意向的用户，可导出单独表格。

⚠ 注意
  · 私密账号无法采集，仅支持公开主页。
  · 用户发布量大（>500 条）时建议分批次采集。
  · 采集到的用户数据请遵守个人信息保护法，不得用于未授权的营销。"""),

            ("④ 评论模板 & 批量评论",
             """【④ 评论模板 & 批量评论】

◆ 功能说明
  准备好多条评论模板，对搜索结果或指定笔记批量随机发送评论。
  支持 DeepSeek AI 改写，让每条评论都有个性化文案。

◆ 使用步骤
  1. 切换到"④ 评论模板 ⚠"标签页
  2. 在文本框中填写评论模板，每行一条（建议 ≥ 10 条保证随机性）
     示例：
       这个真的超好用！
       分享给我闺蜜了～
       已收藏，下次一定试试
  3. 勾选"AI 改写"可让 DeepSeek 自动生成个性化版本（需配置 AI）
  4. 返回"① 搜索"搜到目标笔记，在结果面板勾选 → 点"批量评论"

◆ 间隔设置
  在"⑤ 互动参数"标签页设置评论最小/最大间隔秒数（推荐 30~90 秒）。

⚠ 注意
  · 评论内容请避免含联系方式、广告词、引流词，否则容易被系统删除或限流。
  · 新账号（<30 天）评论数量每日建议 ≤ 15 条。
  · 重复使用相同评论会被识别，务必至少准备 10 条以上不同模板。
  · ⚠ 标签表示此功能属于高风险互动功能，使用时需格外谨慎。"""),

            ("⑤ 互动参数（关注/点赞）",
             """【⑤ 互动参数（关注/点赞）】

◆ 功能说明
  设置批量点赞、关注操作的节奏参数，在结果面板勾选笔记后执行批量互动。

◆ 参数说明
  关注/点赞 间隔最小  → 相邻两次操作之间的最短等待秒数（推荐 30）
  关注/点赞 间隔最大  → 最长等待秒数（推荐 90）
  单次上限            → 本次批量操作最多执行多少条（推荐 ≤ 30）

◆ 使用步骤
  1. 在"⑤ 互动参数 ⚠"标签设好间隔和上限
  2. 到"① 搜索"或"⑥ 爆品发现"结果面板，勾选目标条目
  3. 点结果面板上方"勾选 → 批量点赞"或"勾选 → 关注作者"

⚠ 注意
  · 新账号（<30 天）每日点赞 ≤ 50 条、关注 ≤ 20 人。
  · 老账号每日点赞 ≤ 200 条、关注 ≤ 80 人。
  · 间隔太短（<15 秒）容易触发官方频率限制，请勿调低。
  · ⚠ 标签功能为高风险互动，请合理控制操作量。"""),
...
◆ 功能说明
  按关键词批量扫描笔记，按点赞/评论/收藏数筛选出爆品内容，帮助选品和竞品分析。

◆ 使用步骤
  1. 切换到"⑥ 爆品发现 🔥"标签页
  2. 输入关键词（如"防晒霜 学生"）
  3. 扫描量：搜索结果总数（建议 50~200）
  4. 取前 N：从扫描结果中筛出排名前 N 的笔记
  5. 设置最低点赞/评论/收藏门槛（过滤低质量内容）
  6. 选择排序方式（按点赞/评论/收藏）
  7. 点"🔥 挖爆品"
...
◆ 结果利用
  · 导出 Excel 进行选品分析
  · 勾选后"批量点赞"为目标笔记互热
  · 参考爆品的话题标签和文案风格创作自己的内容

⚠ 注意
  · 扫描量越大耗时越长，100 条约需 2~5 分钟。
  · 爆品数据仅反映当前时间点，建议隔日再跑一次对比趋势。"""),

            ("⑦ 自动养号",
             """【⑦ 自动养号 🌱】

◆ 功能说明
  模拟真实用户行为：自动浏览推荐流 → 随机进笔记 → 阅读正文和评论 → 偶尔点赞/收藏。
  用于提升账号活跃度，降低被风控的概率，新账号必做。

◆ 使用步骤
  1. 切换到"⑦ 自动养号 🌱"标签页
  2. 设置时长（分钟）、点赞概率（%）、收藏概率（%）
  3. 点"🌱 开始养号"
  4. 浏览器会自动操作，不要手动干预，可最小化窗口

◆ 参数建议
  新号（注册 < 7 天）：时长 60 分钟、点赞 5%、收藏 0%
  普通老号：          时长 30 分钟、点赞 15%、收藏 5%
  待激活账号：        每天早晚各跑一次，连续 3~7 天

⚠ 注意
  · 养号期间避免同时执行其他操作（评论/关注等）。
  · 浏览器窗口必须保持打开状态，最小化可以但不能关闭。
  · 不要把点赞概率调太高（>30%），否则模式太机械。"""),

            ("⑧ 发布笔记",
             """【⑧ 发布笔记 📝】

◆ 功能说明
  在软件内撰写笔记（图文/视频/长文），一键发布到单个或多个账号。
  支持 DeepSeek AI 自动生成标题+正文。

◆ 使用步骤
  1. 切换到"⑧ 发布笔记 📝"标签页
  2. 选择类型：图文 / 视频 / 长文
  3. 填写标题、正文、话题标签（多个标签用逗号分隔）
  4. 图文/视频：点"+ 添加"选择本地图片或视频文件
  5. 勾选要发布的账号（右侧账号列表）
  6. 点"📝 发布到选中账号"

◆ AI 一键生成
  · 在"✨ AI 生成主题"框填写主题（如"夏日防晒"）
  · 填写风格（如"轻松活泼"），点"✨ AI 生成内容"
  · 系统自动填入标题、正文和推荐话题标签

◆ 多账号发布
  · 点"🔄 刷新账号列表"确保勾选列表最新
  · 多账号发布时每个账号之间随机等待 30~60 秒，降低关联风险

⚠ 注意
  · 图片建议比例 3:4 或 1:1，大小 ≤ 20 MB/张，最多 9 张。
  · 视频建议 MP4 格式，时长 ≤ 15 分钟。
  · 话题标签填真实热门话题效果更好，可在小红书搜索"#xx"验证热度。
  · 同一内容发布到多个账号会被平台检测为矩阵/搬运，请确保内容有差异。"""),

            ("⑨ 自动搬运",
             """【⑨ 自动搬运 🤖】

◆ 功能说明
  搜索关键词 → 自动下载高赞笔记素材 → AI 改写文案 → 图片去重处理 → 发布。
  支持半自动（审核后发布）和全自动两种模式。

◆ 使用步骤
  1. 切换到"⑨ 自动搬运 🤖"标签页
  2. 填写搜索关键词和素材类型
  3. 搬运数量：每次处理几篇（建议 3~5 篇/天/账号）
  4. 设置最低点赞门槛（建议 1000+，保证素材质量）
  5. 图片处理选项全部勾选（加噪点 + 微裁边 + 随机文件名）
  6. 发布模式选"半自动"（推荐）
  7. 点"🤖 开始搬运（生成预览）"

◆ 半自动 vs 全自动
  半自动：为每篇笔记生成预览，你逐一确认后才发布（推荐，可控风险）
  全自动：自动依次发布全部，按设定间隔（默认 300~900 秒）执行

⚠ 法律警告
  · 搬运他人原创作品涉嫌侵犯著作权，仅限用于自有素材或灵感参考。
  · 不得搬运含人脸/肖像的内容，涉及肖像权纠纷。
  · 图片处理（加噪点/微裁边）仅做去重，无法免除著作权责任。
  · 建议仅对自己拍摄或已获授权的素材使用此功能。"""),

            ("⏰ 定时任务",
             """【⏰ 定时任务管理】

◆ 功能说明
  设定固定时间或周期，让软件自动执行养号、搜索点赞、关注等任务，无需手动干预。
...
⚠ 注意
  · 定时任务只在软件运行期间生效，关闭软件后任务暂停。
  · 建议每个账号每天养号 1~2 次，点赞/关注各不超过 1 次。
  · 任务执行结果可在主界面日志查看。"""),
...
◆ 功能说明
  统一管理多个代理 IP，为不同账号自动分配，避免多账号共用 IP 被关联风控。
...
⚠ 注意
  · 每个账号建议独享一个专属 IP，不要共用。
  · 更换 IP 后建议重新登录或运行一次养号热身。
  · 免费代理不稳定，建议使用付费住宅 IP（品质更高）。
  · 点"🔍 检测IP"可验证当前账号实际出口 IP。"""),

            ("🤖 DeepSeek AI 配置",
             """【🤖 DeepSeek AI 配置】

◆ 功能说明
  配置 DeepSeek API Key，为评论改写、文案生成、自动搬运提供 AI 支持。

◆ 获取 API Key
  1. 访问 https://platform.deepseek.com/
  2. 注册账号 → 进入控制台 → API Keys → 创建新 Key
  3. 复制 Key（sk-xxxxxxxxxxxxxxxx）

◆ 配置步骤
  1. 菜单"⚙设置 → DeepSeek AI"
  2. 选择"自定义 Key"并粘贴 API Key
  3. 模型一般选默认 deepseek-chat 即可
  4. 点"测试连接"验证，出现✓表示配置成功
...
⚠ 注意
  · 若不配置自定义 Key，系统会使用平台共享 Key（有调用频率限制）。
  · API 调用按量收费，建议设置 DeepSeek 控制台的月度用量上限。"""),

            ("🗄 MySQL 数据库",
             """【🗄 MySQL 数据库配置】

◆ 功能说明
  连接 MySQL 数据库，实现采集数据持久化存储、历史去重、操作统计。
  不配置也可正常使用，但去重和历史记录功能不可用。

◆ 配置步骤
  1. 菜单"⚙设置 → MySQL 数据库"
  2. 填写主机（本地填 127.0.0.1，云服务器填 IP）
  3. 填写端口（默认 3306）、用户名、密码、数据库名
  4. 点"🧪 测试连接"确认能连上
  5. 点"🔨 创建表"初始化数据结构（首次必做）
  6. 点"💾 保存"

◆ 数据库功能
  · 📊 今日操作统计：菜单"📊数据 → 今日操作统计"
  · 🗄 自动去重：评论/点赞时跳过已操作过的笔记/用户

⚠ 注意
  · 如使用云服务器 MySQL，需确保 3306 端口在安全组中对本机 IP 开放。
  · 本地使用可安装 XAMPP 或 MySQL Community 8.x。
  · 数据库文件请定期备份（菜单 📦备份 → 备份所有数据）。"""),

            ("🛡 安全功能",
             """【🛡 安全功能】

◆ 清除账号风控警报
  用途：账号出现异常提示（如"操作频繁"）后，清除本地风控记录，让系统重新评估。
  操作：菜单"🛡安全 → 清除当前账号风控警报"
  ⚠ 注意：只是清本地记录，平台侧的限制需靠停止操作 + 养号来恢复。

◆ 重置浏览器指纹
  用途：切换 UA、Canvas、WebGL 等浏览器指纹，降低账号关联风险。
  操作：菜单"🛡安全 → 重置当前账号浏览器指纹"
  ⚠ 注意：重置后建议重新登录账号，确保新指纹与登录状态绑定。

◆ 删除账号
  操作：菜单"🛡安全 → 删除当前账号"
  ⚠ 注意：删除后账号配置、Cookie、指纹全部清除，不可恢复。

◆ 最佳安全实践
  1. 每个账号使用独立代理 IP（住宅 IP 优先）
  2. 新账号至少养号 7 天再开始互动操作
  3. 单日操作量不超过建议值（见各功能注意事项）
  4. 避免 24 小时连续运行同一账号
  5. 定期（每周）重置一次浏览器指纹"""),
...
⚠ 注意
  · 备份文件包含账号 Cookie，请妥善保管，不要泄露给他人。
  · 换电脑使用时：先安装软件 → 激活授权 → 然后恢复备份即可还原全部账号。
  · 建议每周备份一次，至少保留最近 3 份备份。"""),
        ]

        # ── 填充目录 ──
        entries = [title for title, _ in HELP]
        for e in entries:
            lb.insert("end", f"  {e}")
        ...
        def _show(idx):
            ...
            for line in lines:
                ...
                if stripped.startswith("【") and stripped.endswith("】"):
                    ...
                    txt.insert("end", line + "\n", "warn")
                elif stripped.startswith("·") or stripped[0:2] in ("1.", "2.", "3.", "4.", "5.", "6.", "7."):
                    txt.insert("end", line + "\n", "tip")
                elif stripped == "":
                    txt.insert("end", "\n")
                else:
